...
const CodingLab = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/code/analyze`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('Is Suspicious:', analysisResult.analysis?.isSuspicious);

      // Check the analysis property of the result
      if (analysisResult.analysis?.isSuspicious) {
        console.log('Code is suspicious, showing popup...');
        // Store the result
        sessionStorage.setItem('pendingSubmission', JSON.stringify(analysisResult));
        // Force state update
      ...
      console.log('Code is not suspicious, proceeding with submission...');
      handleSubmissionSuccess(analysisResult);
    } catch (error) {
        ...
        <div className="popup-overlay" style={{ zIndex: 9999 }}>
          <div className="popup-content">
            <div className="popup-message">
              Alert: Our analysis of your code indicates that your code is partly or wholly generated by an LLM.We could be mistaken regarding this. If you have written the code by yourself and this is a false alarm, please proceed with your submission.
            </div>
            <div className="popup-buttons">
              ...
              <button
                className="popup-button popup-button-proceed"
                onClick={() => {
                  console.log('Proceed clicked');
                  setShowLLMPopup(false);
                  const pendingResult = JSON.parse(sessionStorage.getItem('pendingSubmission'));
                  handleSubmissionSuccess(pendingResult);
                  sessionStorage.removeItem('pendingSubmission');
                }}
              >
                Proceed
